        ...
        .once('value').then(async function(snap) {
            if (snap.val() === null ) {
                ...
                try {
                   ...
                   await message.guild.channels.find(log => log.id === '717103392878493787')
			.send(`não pude enviar mensagem no pv para ${message.author.username}`);
                }
...
if (cmd.startsWith(`${prefix}setlevel`)) {
    ...
    database.ref(`Level/${userLevel.id}`)
        ...
        .setDescription(`${message.author} setou o nivel de ${userLevel} para ${newLevel}`);
...
if (cmd.startsWith(`${prefix}coloradd`)) {
    ...
    try {
        ...
        message.channel.reply(`Algo inesperado aconteceu. caso o erro persista, fale com ${guild.owner}`);
    }
}
